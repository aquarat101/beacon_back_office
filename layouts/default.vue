...
const menuItems = computed(() => {
    ...
    if (role === ROLES.SUPER_ADMIN) {
        ...
        return [
            ...
            {
                name: "Schools",
                path: `/schools/detail/${schoolId.value}`,
    ...
    <div class="flex">
        <aside class="w-64 bg-white shadow flex flex-col fixed top-0 left-0 h-screen">
            ...
            <div class="mt-auto border-t p-4 flex items-center gap-2 text-sm text-gray-600">
                ...
                <!-- ข้อความชื่อ/อีเมล ให้ flex-grow และ truncate -->
                <div class="flex flex-col overflow-hidden">
                    <div class="font-medium truncate max-w-[150px]">{{ user.name || "Admin" }}</div>
                    <div class="text-xs truncate max-w-[150px]">{{ user.email || "admingmail.com" }}</div>
                </div>

                <!-- ปุ่ม logout -->
                <button class="ml-auto flex-shrink-0 text-gray-500 hover:text-black" @click="logout">
