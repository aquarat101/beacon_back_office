        ...
        <div class="content-bar">
            <div class="search-bar">
                <div class="box">
                    ...
                    <div class="button">
                        <img :style="{ height: '16px', width: '16px' }" :src="'/images/export.png'" />
                        <a> Export </a>
                    </div>
                </div>
                <div class="updated-time">
                    <img :style="{ height: '16px', width: '16px' }" :src="'/images/clock.png'" />
                    <a> Updated 15s ago </a>
                </div>
            </div>
            <div class="card-bar">
                <div v-for="item in items" :key="item.id" class="card" :style="{ height: '201px', width: '266px' }">
            ...
            <div class="card-bar">
                <div v-for="item in items_2" :key="item.id" class="card" :style="{
                    gap: '24px',
                    height: '143px',
                    width: '558px',
                    flexDirection: 'row',
            ...
            <div class="three-cards">
                ...
                <div :style="{ display: 'flex', flexDirection: 'column' }">
                    ...
                    <div class="card" :style="{ height: '397px', width: '558px', padding: '20px', gap: '0px' }">
                        <div
                            :style="{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '8px' }">
                            <a :style="{ fontSize: '20px', fontWeight: 'bold', color: '#848484' }"> Alerts Heatmap (Week
                                by Hour) </a>
                        ...
                        <div :style="{ fontSize: '14px', color: '#848A90', paddingBottom: '6px' }">
                            <strong :style="{ color: 'black' }"> {{ peakHour.value.toLocaleString() }}</strong> visitors
                            in peak hour
                        </div>

                        <!-- Color legend -->
                        <div
                            :style="{ display: 'flex', gap: '16px', fontSize: '12px', color: '#848A90', paddingBottom: '6px' }">
                            <div :style="{ display: 'flex', alignItems: 'center', gap: '4px' }">
                                <div
                                    :style="{ width: '12px', height: '12px', backgroundColor: '#80A4FF', borderRadius: '25%' }">
                                </div>
                                <span>3,000+</span>
                            </div>
                            <div :style="{ display: 'flex', alignItems: 'center', gap: '4px' }">
                                <div
                                    :style="{ width: '12px', height: '12px', backgroundColor: '#B9CDFF', borderRadius: '25%' }">
                                </div>
                                <span>1,000-2,000</span>
                            </div>
                            <div :style="{ display: 'flex', alignItems: 'center', gap: '4px' }">
                                <div
                                    :style="{ width: '12px', height: '12px', backgroundColor: '#EBF6FF', borderRadius: '25%' }">
                                </div>
                                <span>&lt;1,000</span>
                            </div>
                        </div>
                        ...
                        <!-- Heatmap Grid -->
                        <div
                            :style="{ display: 'flex', flexDirection: 'column', width: '510px', height: '247px', gap: '4px' }">
                            <!-- Heatmap rows -->
                            <div v-for="hour in hours" :key="hour"
                                :style="{ display: 'flex', gap: '4px', alignItems: 'center', marginBottom: '2px' }">
                                <!-- Hour label (Y-axis) -->
                                <div
                                    :style="{ width: '45px', fontSize: '12px', color: '#848A90', textAlign: 'right', paddingRight: '8px', fontWeight: '500' }">
                                    {{ hour }}
                                </div>

                                <!-- Heatmap cells -->
                                <div v-for="day in heatmapData" :key="`${day.day}-${hour}`" :style="{
                                    width: '62.14px',
                                    height: '28.57px',
                                    ...
                                    marginRight: '0px'
                                }" @mouseover="$event.target.style.opacity = '0.8'" @mouseout="$event.target.style.opacity = '1'"
                ...
                <div class="card" :style="{ height: '564px', width: '558px', gap: '0px' }">
                    ...
                    <div :style="{ display: 'flex', flexDirection: 'column', paddingTop: '24px' }">
                        <div :style="{ display: 'flex', flexDirection: 'column', gap: '4px' }">
                            <a v-for="item in coordinates" :key="item.id"
                                :style="{ display: 'flex', alignItems: 'center', gap: '0px' }"> <span> <img
                                        :src="'/images/pin.png'" :style="{ width: '13.46px', height: '18.36px' }" />
                                </span> <span :style="{ fontSize: '14px', color: '#2AA937', paddingLeft: '8px' }"> {{
                                    item.name }} <span :style="{ color: '#848A90', paddingLeft: '8px' }"> ({{
                                        item.latitude }}, {{ item.longtitude }}) </span> </span> </a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-gray-100">
                            <tr :style="{ height: '40px', color: '#848A90' }">
                                <th @click="sortTable('date')" :style="{ paddingLeft: '12px', cursor: 'pointer' }">
                                ...
                                <th @click="sortTable('location_name')" :style="{ cursor: 'pointer' }">
                                    <!-- Location Name {{ sortColumn === 'location_name' ? (sortDirection === 'asc' ? '▲' : '▼') : '' }} -->
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="item in table_items" :key="item.id"
