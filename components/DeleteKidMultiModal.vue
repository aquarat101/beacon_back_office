...
const props = defineProps({
    modelValue: Boolean,
    kid: {        // สำหรับ single delete
        type: Object,
        default: () => ({ id: '', name: '' })
    },
    kids: {       // สำหรับ multi delete
        type: Array,
        default: () => []
    }
})
...
async function confirmDelete() {
    const targets = props.kids.length > 0 ? props.kids : props.kid.id ? [props.kid] : []
    ...
    try {
        for (const k of targets) {
            ...
            if (!res.ok) {
                ...
                alert(`Failed to delete "${k.name}": ${json.message || 'Unknown error'}`)
                return
            }
        }
...
const kidNames = computed(() => {
    if (props.kids.length > 0) return props.kids.map(k => `"${k.name}"`).join(", ")
