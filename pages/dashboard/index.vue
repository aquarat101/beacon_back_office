...
const { dashBoardData, fetchDashBoardData } = useDashBoard(config.apiDomain);

async function intDashBoardData() {
  await fetchDashBoardData();

  const mappingRow1 = {
    1: "totalStudents",
    2: "totalActiveStudents",
    3: "totalStudents",
    4: "totalUser",
  };

  const mappingRow2 = {
    1: "totalNewStudentsToday",
    2: "totalNewUsersToday",
  };

  itemRow1.value.forEach((item) => {
    const key = mappingRow1[item.id];
    if (key && dashBoardData.value?.[key] !== undefined) {
      item.number = dashBoardData.value[key].toString();
    }
  });

  itemRow2.value.forEach((item) => {
    const key = mappingRow2[item.id];
    if (key && dashBoardData.value?.[key] !== undefined) {
      item.number = dashBoardData.value[key].toString();
    }
  });
    ...
    <div class="content-bar">
      <div class="search-bar">
        <div class="box">
          <select
            class="search-button"
            :style="{ width: '447px', fontSize: '14px' }"
          >
            <option value="" disabled selected>School</option>
          </select>
          <select
            class="search-button"
            :style="{ width: '257px', fontSize: '14px' }"
          >
      ...
      <div class="three-cards">
        <div :style="{ display: 'flex', flexDirection: 'column' }">
          ...
          >
            ...
            <div
              :style="{ display: 'flex', flexDirection: 'column', gap: '8px' }"
            >
              <a class="head"> {{ itemRow3.head }} </a>
              <a class="number"> {{ itemRow3.number }} </a>
              <div class="percent">
                <span :style="{ color: '#2AA937' }">
                  {{ itemRow3.percent }}
                </span>
                from last month
              </div>
          ...
          >
            ...
            >
              ...
              >
                Alerts Heatmap (Week by Hour)
              </a>
              <select
                class="search-button"
                :style="{ width: '127px', height: '40px', fontSize: '14px' }"
              >
            ...
            >
              <strong :style="{ color: 'black' }">
                {{ peakHour.value.toLocaleString() }}</strong
              >
        ...
        <div
          class="card"
          :style="{ height: '564px', width: '558px', gap: '0px' }"
        >
          <a class="sc-location" :style="{ paddingBottom: '24px' }">
            School Locations
          </a>
            ...
            >
              ...
              >
                <span>
                  <img
                    :src="'/images/pin.png'"
                    :style="{ width: '13.46px', height: '18.36px' }"
                  />
                </span>
                <span
                  :style="{
                ...
                >
                  {{ item.name }}
                  <span :style="{ color: '#848A90', paddingLeft: '8px' }">
      ...
      <div class="card" :style="{ height: '534px', width: '1137px' }">
        ...
        >
          <a
            :style="{ fontSize: '20px', fontWeight: 'bold', color: '#848484' }"
          >
            Latest Alerts
          </a>
          <select
            class="search-button"
            :style="{ width: '93px', height: '40px', fontSize: '14px' }"
          >
        ...
        >
          <table class="w-full text-left border-collapse">
            ...
            <tbody>
              ...
              >
                <td :style="{ paddingLeft: '12px', color: '#848A90' }">
                  {{ item.date }}
                </td>
