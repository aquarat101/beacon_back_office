...
const handleSave = async () => {
  try {
    ...
    const response = await fetch(
      `${config.apiDomain}/schools/update/${form.value.id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
...
<template>
  <div class="p-6">
    ...
    <button @click="goBack()" class="text-blue-500 mb-4 text-lg">
      &lt; Back
    </button>
    ...
    <div v-else-if="form" class="bg-white shadow rounded-xl p-6">
      ...
      <div class="flex items-center mb-6">
        <div class="relative">
          ...
          />
          <button
            class="absolute top-0 right-0 bg-blue-500 text-white p-1 rounded-full shadow"
          >
