...
const allStaffSelected = computed({
  get: () =>
    ...
    paginatedStaffs.value.every((s) => selectedStaffs.value.includes(s.id)),
  set: (val) => {
    const ids = paginatedStaffs.value.map((s) => s.id);
    if (val)
      selectedStaffs.value = Array.from(
        new Set([...selectedStaffs.value, ...ids])
      );
    else
...
const allStudentSelected = computed({
  get: () =>
    ...
    paginatedStudents.value.every((s) => selectedStudents.value.includes(s.id)),
  set: (val) => {
    const ids = paginatedStudents.value.map((s) => s.id);
    if (val)
      selectedStudents.value = Array.from(
...
function toggleStaffSelection(id) {
  if (selectedStaffs.value.includes(id))
    selectedStaffs.value = selectedStaffs.value.filter((i) => i !== id);
  else selectedStaffs.value.push(id);
}

function toggleStudentSelection(id) {
  if (selectedStudents.value.includes(id))
    selectedStudents.value = selectedStudents.value.filter((i) => i !== id);
...
const filteredStudents = computed(() => {
  return students.value.filter((s) => {
    const matchesSearch =
      ...
      s.deviceName
        .toLowerCase()
        .includes(studentSearchActive.value.toLowerCase()) ||
      s.serialNumber
        .toLowerCase()
        .includes(studentSearchActive.value.toLowerCase());
...
<template>
  <div class="h-screen p-6">
    ...
    <div class="flex border-b mb-5">
      ...
      <button
        :class="
          currentTab === 'staff' ? 'border-b-2 border-blue-500 pb-2' : 'pb-2'
        "
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
          <table class="w-full text-left border-collapse">
            <thead class="bg-gray-100">
              <tr>
                <th class="p-3">
                  <input type="checkbox" v-model="allStaffSelected" />
                </th>
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
          <table class="w-full text-left border-collapse">
            <thead class="bg-gray-100">
              <tr>
                <th class="p-3">
                  <input type="checkbox" v-model="allStudentSelected" />
                </th>
                <th class="p-3 text-center">Action</th>
