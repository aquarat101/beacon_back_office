...
const props = defineProps({
    modelValue: Boolean,
    schoolUsers: { // array ของ staff/student ที่เลือก
        type: Array,
        default: () => [] // [{ id: '', name: '' }]
    }
})
...
async function confirmDelete() {
    ...
    try {
        ...
        for (const user of props.schoolUsers) {
            const res = await fetch(`${config.apiDomain}/schools/deleteUser/${user.id}`, {
                method: 'DELETE'
            })
            ...
            <div class="flex gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm">
                        ...
                        <template v-if="props.schoolUsers.length === 1">this user?</template>
